...
class Skill():
    ...
    @staticmethod
    def generate_skill_name_from_save_string(save_string:str):
        '''Generates the skill from the save string in the cat data'''
        if not save_string:
            return None
        ...
        return Skill(SkillPath[split_values[0]])
...
class CatSkills:
    ...
    def get_skill_save_name(self):
        primary = ""
        secondary = ""
        if self.primary:
            if self.primary in SkillPath.RUNNER:
                primary = "RUNNER"
            elif self.primary in SkillPath.TEACHER:
                primary = "TEACHER"
            elif self.primary in SkillPath.HUNTER:
                primary = "HUNTER"
            elif self.primary in SkillPath.FIGHTER:
                primary = "FIGHTER"
            elif self.primary in SkillPath.CLIMBER:
                primary = "CLIMBER"
            elif self.primary in SkillPath.SWIMMER:
                primary = "SWIMMER"
            elif self.primary in SkillPath.SPEAKER:
                primary = "SPEAKER"
            elif self.primary in SkillPath.MEDIATOR:
                primary = "MEDIATOR"
            elif self.primary in SkillPath.CLEVER:
                primary = "CLEVER"
            elif self.primary in SkillPath.INSIGHTFUL:
                primary = "INSIGHTFUL"
            elif self.primary in SkillPath.SENSE:
                primary = "SENSE"
            elif self.primary in SkillPath.KIT:
                primary = "KIT"
            elif self.primary in SkillPath.STORY:
                primary = "STORY"
            elif self.primary in SkillPath.LORE:
                primary = "LORE"
            elif self.primary in SkillPath.CAMP:
                primary = "CAMP"
            elif self.primary in SkillPath.HEALER:
                primary = "HEALER"
            elif self.primary in SkillPath.STAR:
                primary = "STAR"
            elif self.primary in SkillPath.DARK:
                primary = "DARK"
            elif self.primary in SkillPath.OMEN:
                primary = "OMEN"
            elif self.primary in SkillPath.DREAM:
                primary = "DREAM"
            elif self.primary in SkillPath.CLAIRVOYANT:
                primary = "CLAIRVOYANT"
            elif self.primary in SkillPath.PROPHET:
                primary = "PROPHET"
            elif self.primary in SkillPath.GHOST:
                primary = "GHOST"

            # NEW SKILLS
                
            elif self.primary in SkillPath.NAVIGATOR:
                primary = "NAVIGATOR"
            elif self.primary in SkillPath.GUARD:
                primary = "GUARD"
            elif self.primary in SkillPath.STRATEGY:
                primary = "STRATEGY"
            elif self.primary in SkillPath.NIMBLE:
                primary = "NIMBLE"
            elif self.primary in SkillPath.FINDER:
                primary = "FINDER"
            elif self.primary in SkillPath.EXPLORER:
                primary = "EXPLORER"
            elif self.primary in SkillPath.COMFORTER:
                primary = "COMFORTER"
            elif self.primary in SkillPath.FISHER:
                primary = "FISHER"
            elif self.primary in SkillPath.RESOURCEFUL:
                primary = "RESOURCEFUL"
            elif self.primary in SkillPath.RESILIENT:
                primary = "RESILIENT"
            elif self.primary in SkillPath.STRONG:
                primary = "STRONG"
            elif self.primary in SkillPath.TALENTED:
                primary = "TALENTED"
            elif self.primary in SkillPath.SPEECH:
                primary = "SPEECH"
            elif self.primary in SkillPath.TEAMWORK:
                primary = "TEAMWORK"

        # NOW SECONDARY SKILLS

        if self.secondary:
            if self.secondary in SkillPath.RUNNER:
                secondary = "RUNNER"
            elif self.secondary in SkillPath.TEACHER:
                secondary = "TEACHER"
            elif self.secondary in SkillPath.HUNTER:
                secondary = "HUNTER"
            elif self.secondary in SkillPath.FIGHTER:
                secondary = "FIGHTER"
            elif self.secondary in SkillPath.CLIMBER:
                secondary = "CLIMBER"
            elif self.secondary in SkillPath.SWIMMER:
                secondary = "SWIMMER"
            elif self.secondary in SkillPath.SPEAKER:
                secondary = "SPEAKER"
            elif self.secondary in SkillPath.MEDIATOR:
                secondary = "MEDIATOR"
            elif self.secondary in SkillPath.CLEVER:
                secondary = "CLEVER"
            elif self.secondary in SkillPath.INSIGHTFUL:
                secondary = "INSIGHTFUL"
            elif self.secondary in SkillPath.SENSE:
                secondary = "SENSE"
            elif self.secondary in SkillPath.KIT:
                secondary = "KIT"
            elif self.secondary in SkillPath.STORY:
                secondary = "STORY"
            elif self.secondary in SkillPath.LORE:
                secondary = "LORE"
            elif self.secondary in SkillPath.CAMP:
                secondary = "CAMP"
            elif self.secondary in SkillPath.HEALER:
                secondary = "HEALER"
            elif self.secondary in SkillPath.STAR:
                secondary = "STAR"
            elif self.secondary in SkillPath.DARK:
                secondary = "DARK"
            elif self.secondary in SkillPath.OMEN:
                secondary = "OMEN"
            elif self.secondary in SkillPath.DREAM:
                secondary = "DREAM"
            elif self.secondary in SkillPath.CLAIRVOYANT:
                secondary = "CLAIRVOYANT"
            elif self.secondary in SkillPath.PROPHET:
                secondary = "PROPHET"
            elif self.secondary in SkillPath.GHOST:
                secondary = "GHOST"

            # NEW SKILLS
                
            elif self.secondary in SkillPath.NAVIGATOR:
                secondary = "NAVIGATOR"
            elif self.secondary in SkillPath.GUARD:
                secondary = "GUARD"
            elif self.secondary in SkillPath.STRATEGY:
                secondary = "STRATEGY"
            elif self.secondary in SkillPath.NIMBLE:
                secondary = "NIMBLE"
            elif self.secondary in SkillPath.FINDER:
                secondary = "FINDER"
            elif self.secondary in SkillPath.EXPLORER:
                secondary = "EXPLORER"
            elif self.secondary in SkillPath.COMFORTER:
                secondary = "COMFORTER"
            elif self.secondary in SkillPath.FISHER:
                secondary = "FISHER"
            elif self.secondary in SkillPath.RESOURCEFUL:
                secondary = "RESOURCEFUL"
            elif self.secondary in SkillPath.RESILIENT:
                secondary = "RESILIENT"
            elif self.secondary in SkillPath.STRONG:
                secondary = "STRONG"
            elif self.secondary in SkillPath.TALENTED:
                secondary = "TALENTED"
            elif self.secondary in SkillPath.SPEECH:
                secondary = "SPEECH"
            elif self.secondary in SkillPath.TEAMWORK:
                secondary = "TEAMWORK"
