...
def create_new_cat(Cat,
    ...
    if not isinstance(age, int):
        if status == "newborn":
            ...
            age = randint(1, 5)
        elif status in ('apprentice', 'medicine cat apprentice', 'mediator apprentice', 'defense apprentice', 'attack apprentice', 'hunt apprentice', 'starteller apprentice'):
            age = randint(6, 11)
        elif status in ('warrior', 'defense', 'attack', 'hunt'):
            age = randint(23, 120)
    ...
    if not status:
        if age == 0:
            ...
            status = choice(["attack apprentice", "defense apprentice", "hunt apprentice"])
        elif age >= 12:
            status = choice(["attack", "defense", "hunt"])
        elif age >= 120:
            status = 'elder'
...
def generate_sprite(cat, life_state=None, scars_hidden=False, acc_hidden=False, always_living=False, 
    ...
    try:
        ...
        # TINTS
        if cat.pelt.tint_category != "none" and cat.pelt.tint_category in sprites.cat_tints["tint_categories"]:
            # Multiply with alpha does not work as you would expect - it just lowers the alpha of the
            # entire surface. To get around this, we first blit the tint onto a white background to dull it,
            # then blit the surface onto the sprite with pygame.BLEND_RGB_MULT
            tint = pygame.Surface((sprites.size, sprites.size)).convert_alpha()
            tint.fill(cat.pelt.true_tint)
        ...
        if not acc_hidden:        
            ...
            elif cat.pelt.accessory in cat.pelt.new_accessories:
                new_sprite.blit(sprites.sprites['acc_new' + cat.pelt.accessory + cat_sprite], (0, 0))
