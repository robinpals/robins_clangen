...
class Patrol():
    ...
    def add_patrol_cats(self, patrol_cats: List[Cat], clan: Clan) -> None:
        ...
        for cat in patrol_cats:
            ...
            if cat.status == 'apprentice' or cat.status == 'medicine cat apprentice' or cat.status == 'starteller apprentice' or cat.status == 'defense apprentice' or cat.status == 'attack apprentice' or cat.status == 'hunt apprentice':
            ...
            if cat.status in ("medicine cat", "medicine cat apprentice", "starteller", "starteller apprentice"):
                if "healer cats" in self.patrol_statuses:
                    self.patrol_statuses["healer cats"] += 1
                else:
                    self.patrol_statuses["healer cats"] = 1
            
            if cat.status in ("apprentice", "defense apprentice", "attack apprentice", "hunt apprentice", "medicine cat apprentice", "starteller apprentice"):
                if "all apprentices" in self.patrol_statuses:
                    self.patrol_statuses["all apprentices"] += 1
                else:
                    self.patrol_statuses["all apprentices"] = 1
                    
            if cat.status in ("warrior", "deputy", "leader", "guide", "defense", "attack", "hunt"):
        ...
        # sets leader as patrol leader
            ...
            self.patrol_leader = self.patrol_cats[index]
        elif "starteller" in self.patrol_status_list:
            index = self.patrol_status_list.index("starteller")
            self.patrol_leader = self.patrol_cats[index]
        elif "starteller apprentice" in self.patrol_status_list:
            index = self.patrol_status_list.index("starteller apprentice")
            self.patrol_leader = self.patrol_cats[index]
        else:
            # Get the oldest cat
